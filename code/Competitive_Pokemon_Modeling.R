...
length(which(pokeData$Comp_Bin==1))

# Create a binary variable for typings
for(i in seq_along(elemental_types)){
  pokeData[elemental_types[i]] <- ifelse(str_detect(pokeData$Types, elemental_types[i]), 1, 0)
}

# Create a binary variable for abilities
for(i in seq_along(abilities_list)){
  pokeData[abilities_list[i]] <- ifelse(str_detect(pokeData$Abilities, abilities_list[i]), 1, 0)
}
